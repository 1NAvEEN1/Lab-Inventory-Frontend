...
const renderTreeNodes = (nodes, level = 0, onNodeClick) => {
  return nodes.map((node) => {
    ...
    return (
      ...
      >
        {hasChildren ? renderTreeNodes(node.children, level + 1, onNodeClick) : null}
      </TreeItem>
    );
  });
};
...
const ParentLocationSelector = ({ value, onChange, error, helperText, disabled }) => {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!isMounted) return;
        setErrorState("Failed to load locations");
        console.error("ParentLocationSelector load error", e);
      } finally {
  ...
  const selectedNode = useMemo(() => {
    console.log("Calculating selectedNode:", { value, flatNodesLength: flatNodes.length });
    if (!value) return null;
    const found = flatNodes.find((node) => String(node.id) === String(value)) || null;
  ...
  const handleTreeNodeClick = (node) => {
    ...
    if (onChange) {
      console.log("Calling onChange with ID:", String(node.id));
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (open && !event.target.closest('[data-parent-location-selector]')) {
        setOpen(false);
      }
    };
      ...
      {open && (
        ...
        >
          {inputValue.trim() ? (
            ...
            <SimpleTreeView
              defaultExpandedItems={flatNodes.map((node) => String(node.id))}
              sx={{
                "& .MuiTreeItem-content": {
                  padding: 0.5,
                },
            ...
            >
              {renderTreeNodes(nodes, 0, handleTreeNodeClick)}
            </SimpleTreeView>
          )}
        </Paper>
      )}
...
ParentLocationSelector.propTypes = {
